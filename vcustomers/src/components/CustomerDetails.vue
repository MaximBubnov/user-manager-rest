<template>
  <div class="details container">
    ...
    <h1 class="page-header">{{customer.lastname}} {{customer.firstname}}
      <span class="pull-right">
        <router-link class="btn btn-default" v-bind:to="'/edit/' + customer.id">Edit</router-link>
        <button class="btn btn-danger" @click="deleteCustomer(customer.id)">Delete</button>
      </span>
    </h1>

    <ul class="list-group">
      <li class="list-group-item"><span class="glyphicon glyphicon-user" aria-hidden="true"></span> {{customer.age}}</li>
    </ul>
...
<script>
  export default {
    ...
    methods: {
      fetchCustomer(id){
        this.$http.get('http://localhost:8080/manage/customers/' + id)
          .then(result => result.json().then(data =>
            this.customer = data));
      },
      deleteCustomer(id) {
        this.$http.delete('http://localhost:8080/manage/' + id)
          .then(data =>
            this.$router.push({path: '/', query: {alert: 'Customer Delete'}}));
      }
    },
    created() {
